...
* **8.4 API Keys & Trazabilidad de Envíos de Correo (`email_logs`):** *(✅ COMPLETADO)*
  * *Descripción:* Modelo `ApiKey` (`auth_api_keys`) para integración programática externa + tabla dedicada `sys_email_logs` para registrar historial de envíos (`to`, `subject`, `template_name`, `status`, error, timestamp, `job_id`, `attempt`), sanitizando y excluyendo estrictamente tokens temporales o URLs secretas por cumplimiento GDPR y seguridad (prevención de Account Takeover).
  * *Envío Asíncrono Resiliente (`emails.send`):* Desacople del envío mediante el job `emails.send` de `sys_jobs` (reemplazando la ejecución síncrona en `modules/email/service.py`), incorporando reintentos con backoff exponencial, propagación de errores para retry y registro automático de estados en `sys_email_logs`. Los payloads de `sys_jobs` omiten el HTML pesado persistiendo únicamente `template_name` y `template_context`.
  * *Gestión de Plantillas Dinámicas (`sys_email_templates`):* Soporte planificado para plantillas en base de datos editables desde backoffice, gobernadas mediante `BaseAuditService` para registrar versiones y diffs `before`/`after` de cada plantilla.
    > ⚠️ **Nota de Transición Técnica (Plantillas):** En el apartado 8.4 (`sys_email_logs` y job `emails.send`), temporalmente se almacena el nombre del archivo de plantilla (`template_name`, evitando almacenar el HTML completo en base de datos). Cuando se implemente el módulo de plantillas dinámicas en base de datos (`sys_email_templates`), se deberá actualizar el 8.4 para asociar directamente el `template_id` (UUID) en lugar del nombre de la plantilla.
...
* **8.5 Passkeys (FIDO2 / WebAuthn - Passwordless Biométrico y Cross-Device QR):** *(⏳ PENDIENTE)*
  * *Descripción:* Autenticación moderna de máxima seguridad resistente al phishing basada en el estándar oficial FIDO2 / WebAuthn utilizando `py_webauthn`:
    - *Criptografía Asimétrica de Clave Pública:* La clave privada nunca sale del hardware del usuario (*Secure Enclave*, chip *Titan*, TPM de Windows, YubiKey). En base de datos solo se persiste la clave pública, el `credential_id`, el contador anti-clonación `sign_count` y el nombre identificativo del dispositivo en la tabla `auth_passkeys`.
    - *Inmunidad contra Phishing:* Vinculación criptográfica obligatoria con el dominio/origen (*Relying Party ID*). Es matemáticamente imposible que una web maliciosa capture o reenvíe credenciales válidas.
    - *Inicio de Sesión Cross-Device (QR + Bluetooth):* Permite acceder desde el navegador de escritorio escaneando un código QR nativo con el móvil, utilizando validación de proximidad física mediante Bluetooth de baja energía (BLE) y confirmación biométrica (FaceID / TouchID).
    - *Gestión Multi-Dispositivo:* El usuario puede registrar y nombrar múltiples llaves (ej: "MacBook Pro TouchID", "iPhone FaceID", "YubiKey 5C"), con soporte para llaves sincronizadas (*Synced Passkeys* en iCloud Keychain / Google Password Manager) o llaves de hardware (*Hardware Security Keys*).
    - *Coexistencia Armónica:* Se integra como método de acceso de primer nivel, manteniendo Magic Links y Contraseña + 2FA TOTP como mecanismos alternativos o de recuperación si el usuario no tiene su dispositivo disponible.
  * *Problema que soluciona:* Máxima comodidad y seguridad sin contraseñas (*passwordless* real), eliminando la fricción de recordar claves y ofreciendo una experiencia idéntica a la de gigantes tecnológicos (Google, Apple, Microsoft Entra ID).
