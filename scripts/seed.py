...
from scripts.seeds.notifications import seed_notifications
...
async def run_all_seeds(session: AsyncSession) -> None:
    ...
    logger.info("👥 [4/6] Creando/actualizando equipos globales y asignaciones...")
    team_map = await seed_teams(session, role_map, superadmin)
    logger.info(f"   ✔ Equipos sincronizados: {len(team_map)}")

    # 5. System Settings
    logger.info("⚙️ [5/6] Creando/actualizando settings dinámicos del sistema...")
    settings_list = await seed_settings(session)
    logger.info(f"   ✔ Settings sincronizados: {len(settings_list)}")

    # 6. Welcome Notification
    logger.info("🔔 [6/6] Creando/verificando notificación de bienvenida...")
    notification = await seed_notifications(session, superadmin)
    if notification and superadmin:
        logger.info(f"   ✔ Notificación configurada para: {superadmin.email}")
    else:
        logger.warning("   ⚠ Notificación omitida (superadmin no disponible)")
